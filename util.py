...
def softmax_to_hard(y_soft, dim):  # the function uses the hard version trick as in gumbel softmax
    index = y_soft.max(dim, keepdim=True)[1]
    y_hard = torch.zeros_like(y_soft, memory_format=torch.legacy_contiguous_format).scatter_(dim, index, 1.0)
    return y_hard - y_soft.detach() + y_soft


def gumbel_softmax(src, index, tau=-1, num_nodes=None):  # modified from torch_geometric.utils import softmax
